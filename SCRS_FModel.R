...
FileName <- 'SCRS_Model Inputs.xlsx'
#
#Reading Input File
...
RunModel <- function(AnalysisType, SimReturn, SimVolatility, ER_Policy_CurrentHires, ScenType){
  ...
  for(i in StartIndex:length(FYE)){
    ...
    UAL_AVA <- UAL_AVA %>% replace(is.na(.), 0)
    ...
    TempValue <- TempValue %>% replace(is.na(.), 0)
    if(TempValue < 0){
    ...
    TotalContPre <- TotalContPre %>% replace(is.na(.), 0)
    TotalContPost <- TotalContPost %>% replace(is.na(.), 0)
    FR_AVA <- FR_AVA %>% replace(is.na(.), 0)
...
x <- RunModel(AnalysisType = 'Deterministic',SimVolatility, 
         ER_Policy_CurrentHires = 'Statutory Rate', ScenType = Scenarios[1])
write_csv(x, "SCRS_RunModel.csv")
